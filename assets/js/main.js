...
$(function () {
  ...
  $("body").on("submit", "._transaction_form", function () {
    ...
    var $previewItems = $itemsTable.find(".sortable");
    
    var groups = [];
    var i = null;
    var item_groups = [];
    $.each($previewItems, function(x, item) {

      var value = $(this).find('[id="group_id"]').val();

      if (i === value || value === '') return ;
      item_groups.push(value);

      $.ajax({
        async: false,
        type: 'post',
        url: admin_url + 'proposals/get_all_categories',
        data: {
            'group_id': $(this).find('[id="group_id"]').val(),
            'token': csrfData['hash']
        },
    ...
    var new_groups = groups.filter(onlyUnique);
    var is_save;
    let allFounded = new_groups.every( ai => item_groups.includes(ai) );
    if (allFounded)
      is_save = true;
    else is_save = false;

    if (new_groups == []) is_save = true;
    if (!is_save) {
      $itemsTable.before(
        '<div class="alert alert-warning mbot20" id="items-warning">' +
        app.lang.item_missed +
...
function initDataTable(
  ...
  var dtSettings = {
    ...
    bDeferRender: true,
    autoWidth: false,
    dom: "<'toolbar'><'row'><'row'<'col-md-7'lB><'col-md-5'f>>rt<'row'<'col-md-4'i><'col-md-8 dataTables_paging'<'#colvis'><'.dt-page-jump'>p>>",
    ...
    fnDrawCallback: function (oSettings) {
      ...
      // $('#invoices-report button.btn-default-dt-options').click(function(e) {
      //   setTimeout( function() {
      //     var $btn = $('<li class="dt-button buttons-excel buttons-html5" tabindex="4" aria-controls="DataTables_Table_0"><a href="#">XML</a></li>');
      //     $btn.click(function() {
      //       $.ajax({
      //         url: url,
      //         type: "get",
      //         success: function(res) {
      //         //   var doc = document.implementation.createDocument(null, "example", null);
      ...
      //       });
      //     });
      //     $("#invoices-report .dt-buttons .dropdown-menu").append($btn);
      //   }, 100);
      // });
    },
    ...
    buttons: get_table_buttons(table),
  };

  function get_table_buttons(table) {
        ...
        buttons: [
          ...
          },
          {
            ...
            action: function ( e, dt, node, config ) {
              var oTable = $('.DataTables_Table_0')[0];
                console.log(dt, '========')
            },
            footer: true
          },
...
// Updates project when action performed form kan ban area eq status changed.
function projects_kanban_update(ui, object) {
  if (object === ui.item.parent()[0]) {
    ...
    setTimeout(function () {
      $.post(admin_url + "projects/update_order", data).done(function (response) {
        update_kan_ban_total_when_moving(ui, data.status);
        projects_kanban();
      });
    }, 200);
...
// Init projects kan ban
function projects_kanban() {
  init_kanban("projects/kanban", projects_kanban_update, ".projects-status", 240, 360);
}
...
function add_item_to_preview(id) {
  ...
  requestGetJSON("invoice_items/get_item_by_id/" + id).done(function (
    ...
    $('.main input[id="group_id"]').val(response.group_id);
    ...
    if(response.manual_description) {
      $('.main textarea[id="manual_description"]').val(
        response.manual_description.replace(/(<|&lt;)br\s*\/*(>|&gt;)/g, " ")
      );
    }
    else {
      $('.main textarea[id="manual_description"]').val('');
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row += '</td><td style="display:none">';
    table_row +=
      '<input hidden id="group_id" value="' + data.group_id + '" name="newitems[' +
      item_key +
      '][group_id]" ></td>';
    table_row +=
    ...
    table_row +=
      '<td><textarea name="newitems[' +
      item_key +
      '][manual_description]" class="form-control item_long_description" rows="5">' +
      (data.manual_description ? data.manual_description : ' ') +
      "</textarea></td>";
    console.log('==============', table_row, data.manual_description);
    ...
    table_row +=
      '<td class="discount"><input type="number" data-toggle="tooltip" min="0" max="100" title="' +
      app.lang.item_field_not_formatted +
      '" onblur="calculate_total();" onchange="calculate_total();" name="newitems[' +
      item_key +
      '][discount]" value="' +
...
function get_item_preview_values() {
  ...
  response.discount = $('.main input[id="discount"]').val();
...
function calculate_total() {
  ...
  $.each(rows, function () {
    ...
    {
    _amount -=   _amount * parseFloat(discount)/100;
    }

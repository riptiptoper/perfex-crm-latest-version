<script>
	...
	initDataTable(table_manage_order_return, admin_url+'omni_sales/table_manage_order_return',[],[], InvoiceServerParams, [0 ,'desc']);

	$('.order_return_sm').DataTable().columns([0]).visible(false, false);

	$('input[name="from_date"], input[name="to_date"], select[name="staff_id[]"], select[name="delivery_id[]"], select[name="rel_type_filter[]"]').on('change', function() {
	...
	function init_order_return(id) {
		"use strict";
		load_small_table_item_proposal(id, '#order_return_sm_view', 'delivery_id', 'omni_sales/view_order_return', '.order_return_sm');
	}

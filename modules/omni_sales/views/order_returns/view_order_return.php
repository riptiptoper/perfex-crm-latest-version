...
<div class="col-md-12 no-padding">
	<div class="panel_s">
								...
								<address>
									...
									<p class="no-mbot">
										<span class="bold">
											...
											<a href="<?php echo admin_url('omni_sales/view_order_detailt/'.$order_return->rel_id) ?>" ><?php echo omni_get_sales_order_code($order_return->rel_id); ?></a>
										</span>
										<h5 class="bold">
										</h5>
									</p>
						...
						<div class="row">
									...
									<table class="table items items-preview estimate-items-preview" data-type="estimate">
										<thead>
											<tr>
												...
												<th width="15%" align="right"><?php echo _l('reason_return'); ?></th>
										...
										<tbody class="ui-sortable">
											...
											foreach ($order_return_detail as $delivery => $order_return_value) {
												...
												$commodity_code = omni_get_commodity_name($order_return_value['commodity_code']) != null ? omni_get_commodity_name($order_return_value['commodity_code'])->commodity_code : '';
												$commodity_name = omni_get_commodity_name($order_return_value['commodity_code']) != null ? omni_get_commodity_name($order_return_value['commodity_code'])->description : '';

												$unit_name = '';
												if(is_numeric($order_return_value['unit_id'])){
													$unit_name = omni_get_unit_type($order_return_value['unit_id']) != null ? ' '.omni_get_unit_type($order_return_value['unit_id'])->unit_name : '';
												}
												...
												<tr>
													...
													<?php echo  omni_render_taxes_html(omni_convert_item_taxes($order_return_value['tax_id'], $order_return_value['tax_rate'], $order_return_value['tax_name']), 15); ?>
													...
													<td class="text-right"><?php echo html_entity_decode($order_return_value['reason_return']) ?></td>
												</tr>
											<?php  } ?>
										</tbody>
									</table>
							...
							<div class="col-md-12">
													...
													if($value['action'] == 'sign'){
														...
														<div class="col-md-3 text-center">
															...
															<?php if($value['approve'] == 1){ 
																...
																<?php if (file_exists(WAREHOUSE_STOCK_EXPORT_MODULE_UPLOAD_FOLDER . $order_return->id . '/signature_'.$value['id'].'.png') ){ ?>

																	<img src="<?php echo site_url('modules/warehouse/uploads/stock_export/'.$order_return->id.'/signature_'.$value['id'].'.png'); ?>" class="img-width-height">
								...
								<?php if($order_return->return_policies_information != ''){ ?>
									<div class="col-md-12 row mtop15">
										<p class="bold text-muted"><?php echo _l('wh_return_policies_information'); ?></p>
		...
		<?php require 'modules/omni_sales/assets/js/order_returns/view_order_return_js.php';?>
	</body>
	</html>
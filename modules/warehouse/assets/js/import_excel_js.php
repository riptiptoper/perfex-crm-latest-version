   ...
   (function($) {
    ...
    "use strict";
      ...
      if('<?php echo html_entity_decode($active_language) ?>' == 'vietnamese')
      {
        ...
        $( "#dowload_variation_file" ).append( '<a href="'+ site_url+'modules/warehouse/uploads/file_sample/Sample_import_variation_commodity_file_en.xlsx" class="btn btn-primary" ><?php echo _l('download_sample') ?></a><hr>' );
...
function uploadfilecsv1(event){
  "use strict";
    ...
    $.ajax({ 
      url: admin_url + 'warehouse/import_file_xlsx_commodity_variation', 
      method: 'post', 
      data: formData, 
      contentType: false, 
      processData: false
      ...
      $("#file_csv1").val(null);
      $("#file_csv1").change();
       $(".panel-body").find("#file_variation_upload_response").html();

      if($(".panel-body").find("#file_variation_upload_response").html() != ''){
        $(".panel-body").find("#file_variation_upload_response").empty();
      };


        $( "#file_variation_upload_response" ).append( "<h4><?php echo _l("_Result") ?></h4><h5><?php echo _l('import_line_number') ?> :"+response.total_rows+" </h5>" );
   

     
        $( "#file_variation_upload_response" ).append( "<h5><?php echo _l('import_line_number_success') ?> :"+response.total_row_success+" </h5>" );



        $( "#file_variation_upload_response" ).append( "<h5><?php echo _l('import_line_number_failed') ?> :"+response.total_row_false+" </h5>" );


      if((response.total_row_false > 0) || (response.total_rows_data_error > 0))
      {
        $( "#file_variation_upload_response" ).append( '<a href="'+site_url +response.filename+'" class="btn btn-warning"  ><?php echo _l('download_file_error') ?></a>' );
      }
      if(response.total_rows < 1){
        alert_float('warning', response.message);
      }

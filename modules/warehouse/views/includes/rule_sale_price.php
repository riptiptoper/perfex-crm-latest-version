...
<div class="row">
  ...
  <div class=" form-group">
    <select onchange="goods_receipt_warehouse_change(this); return false" name="warehouse_receive_return_order" class="selectpicker" id="warehouse_receive_return_order" data-width="100%" data-none-selected-text="<?php echo _l('warehouse_name'); ?>"> 
      <option value=""></option>
      <?php foreach($warehouses as $wh){ ?>
        <option value="<?php echo html_entity_decode($wh['warehouse_id']); ?>" <?php if(get_warehouse_option('warehouse_receive_return_order') == $wh['warehouse_id']){ echo 'selected';} ?> ><?php echo html_entity_decode($wh['warehouse_code'].'_'.$wh['warehouse_name']); ?></option>
      <?php } ?>
    </select>
  </div>  

...
class Migration_Version_119 extends App_module_migration
{
    ...
    public function up()
    {
        ...
        if ( ! $this->db->field_exists('recurring', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 
        ...
        if ( ! $this->db->field_exists('recurring_type', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 
        ...
        if ( ! $this->db->field_exists('repeat_every', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 
        ...
        if ( ! $this->db->field_exists('custom_recurring', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 
        ...
        if ( ! $this->db->field_exists('cycles', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 
        ...
        if ( ! $this->db->field_exists('total_cycles', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 
        ...
        if ( ! $this->db->field_exists('last_recurring_date', $table)) {
            $CI->db->query(
                "ALTER TABLE {$table} 

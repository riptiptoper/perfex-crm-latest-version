    ...
    <div class="panel_s">
      <div class="panel-body">
        <div class="col-md-12">
          ...
          <div class="relative max-height-300" >
            <canvas class="chart" height="300" id="contracts-by-type-chart"></canvas>
          </div>
        </div>
        <div class="clearfix"></div>
        <table class="table dt-table dt-inline dataTable no-footer table-contracts" data-order-col="4" data-order-type="asc">
          <thead>
            <tr>
          ...
          <tbody>
            <?php foreach($contracts as $contract){
              ...
              ?>
              <tr class="<?php echo html_entity_decode($expiry_class); ?>">
                <td>
                  <?php
                  echo '<a href="'.site_url('sales_agent/purchase_contract/index/'.$contract['id'].'/'.$contract['hash']).'" class="td-contract-url">'.$contract['subject'].'</a>';
                  ?>
                </td>
                <td><?php echo html_entity_decode($contract['type_name']); ?></td>
                <td>
                  <?php
               ...
               <td data-order="<?php echo html_entity_decode($contract['datestart']); ?>"><?php echo _d($contract['datestart']); ?></td>
               <td data-order="<?php echo html_entity_decode($contract['dateend']); ?>"><?php echo _d($contract['dateend']); ?></td>
...
<script>
  var contracts_by_type = '<?php echo html_entity_decode($contracts_by_type_chart); ?>';

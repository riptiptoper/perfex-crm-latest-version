...
/**
 * This class describes an authentication sale agent model.
 */
class Authentication_sale_agent_model extends App_Model
{
    /**
    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            $this->db->where('userid IN (SELECT userid FROM '.db_prefix().'clients where client_type = "agent")');
            ...
            if ($staff == true) {
                ...
                hooks()->do_action('before_affiliate_login', [
                    'email'           => $email,
                    'sale_agent_user_id'          => $user->userid,
                    'sale_agent_contact_user_id'   => $user->id,
    ...
    public function logout($staff = true)
    {
        ...
        if (is_sale_agent_logged_in()) {
            hooks()->do_action('before_sale_agent_logout', get_sa_contact_user_id());

            $this->session->unset_userdata('sale_agent_user_id');
            $this->session->unset_userdata('sale_agent_logged_in');
            $this->session->unset_userdata('sale_agent_contact_user_id');
        }
    ...
    {
        if (!is_sale_agent_logged_in() && !is_staff_logged_in()) {
            $this->load->helper('cookie');
            if ($cookie = get_cookie('autologin', true)) {
                $data = unserialize($cookie);

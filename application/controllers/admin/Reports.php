...
class Reports extends AdminController
{
    ...
    public function report_document()
    {
        $data['groups'] = $this->db->get(db_prefix() . 'documentgroups')->result_array();
        $data['title']                           = _l('report_document');
        $this->load->view('admin/reports/report_document', $data);
    }
    ...
    public function add_group()
    {
        ...
        $this->db->insert(db_prefix() . 'documentgroups', $data);
    ...
    public function edit_group()
    {
        ...
        unset($data['group_id']);
        $this->db->update(db_prefix() . 'documentgroups', $data);
        echo 1;
    }
    ...
    public function save_field_data()
    {
        ...
        $data['fieldto'] = 'reports';
        $this->db->where('group_id', $data['group_id']);
        $this->db->where('relid', $data['relid']);
        $this->db->where('fieldid', $data['fieldid']);
        $this->db->where('fieldto', 'reports');
        $total_rows = $this->db->count_all_results(db_prefix() . 'customfieldsvalues');
        ...
        if ($total_rows > 0) {
            ...
            $this->db->where('fieldid', $data['fieldid']);
            $this->db->where('fieldto', 'reports');
            $this->db->update(db_prefix().'customfieldsvalues', ['value' => $data['value']]);
        }
        else {
            $this->db->insert(db_prefix().'customfieldsvalues', $data);
        }
    }
    ...
    public function delete_group()
    {
        ...
        $this->db->delete(db_prefix() . 'documentgroups');
        echo 1;
    }
    ...
    public function update_group()
    {
        ...
        $this->db->where('id', $id);
        $data['group'] = $this->db->get(db_prefix() . 'documentgroups')->row();
        $lists = get_custom_fields('reports');
        foreach ($lists as $list)
        {
            $selected = ' ';
            if (in_array($list['id'], json_decode($data['group']->value)))
                $selected = "selected";
            $html .=  '<option ' . $selected . ' value="' . $list['id'] . '">' . $list['name'] . '</option>';
        }
        $data['html'] = $html;

...
class Projects_model extends App_Model
{
    ...
    public function update_order($data)
    {
        ...
        if ($data['status'] == "4")
        {
            $update_data['date_finished'] = date('Y-m-d H:m:s');
        }
        
        if ($status == "4")
        {
            $update_data['date_finished'] = null;
        }

        $update_data['status'] = $data['status'];
        $this->db->where('id', $data['projectid']);
        $this->db->update(db_prefix() . 'projects', $update_data);
    }
    ...
    {
        $this->db->select('*, (SELECT GROUP_CONCAT(id SEPARATOR ",") FROM ' . db_prefix() . 'tasks WHERE rel_type="project" AND rel_id=' . $this->db->escape_str($project_id) . ' and milestone=' . db_prefix() . 'milestones.id) as task_ids, (SELECT COUNT(id) FROM ' . db_prefix() . 'tasks WHERE rel_type="project" AND rel_id=' . $this->db->escape_str($project_id) . ' and milestone=' . db_prefix() . 'milestones.id) as total_tasks, (SELECT COUNT(id) FROM ' . db_prefix() . 'tasks WHERE rel_type="project" AND rel_id=' . $this->db->escape_str($project_id) . ' and milestone=' . db_prefix() . 'milestones.id AND status=5) as total_finished_tasks');
        $this->db->where('project_id', $project_id);
        $this->db->order_by('milestone_order', 'ASC');
        $this->db->where($where);
        ...
        foreach ($milestones as $milestone) {
            ...
            {
                $task_total_amount_by_id = 0;
                foreach (explode(', ', $milestone['task_ids']) as $task_id)
                {
                    ...
                    $task_total_amount_by_id += $data->total_hours * $data->hourly_rate;
                }
            }
            $milestones[$i]['total_billable_amount'] = $task_total_amount_by_id;
            $i++;
        }
    ...
    public function add($data)
    {
        ...
        $settingPercentValues = $this->db->get(db_prefix() . 'settingprojectpercent')->row();
        $data['vanzare']      = $settingPercentValues->vanzare;
        $data['proiectare']   = $settingPercentValues->proiectare;
        $data['bc']           = $settingPercentValues->bc;
        $data['productie']    = $settingPercentValues->productie;
        $data['livrare']      = $settingPercentValues->livrare;
        $data['montaj']       = $settingPercentValues->montaj;
        $data['profit']       = $settingPercentValues->profit;
        ...
        if (isset($data['project_mounters'])) {
            $project_mounters = $data['project_mounters'];
            unset($data['project_mounters']);
        }
        if (isset($data['project_designers'])) {
            $project_designers = $data['project_designers'];
            unset($data['project_designers']);
        }
        if (isset($data['project_technologists'])) {
            $project_technologists = $data['project_technologists'];
            unset($data['project_technologists']);
        }
        ...
        if ($insert_id) {
            ...
            if (isset($project_mounters)) {
                $_pm['project_mounters'] = $project_mounters;
                $this->add_edit_mounter_members($_pm, $insert_id);
            }

            if (isset($project_designers)) {
                $_pm['project_designers'] = $project_designers;
                $this->add_edit_designer_members($_pm, $insert_id);
            }

            if (isset($project_technologists)) {
                $_pm['project_technologists'] = $project_technologists;
                $this->add_edit_technologist_members($_pm, $insert_id);
            }
    ...
    public function update($data, $id)
    {
        ...
        if (isset($project_mounters)) {
            $_pm['project_mounters'] = $project_mounters;
            $mounter = $this->add_edit_mounter_members($_pm, $id);
        }
        $_pm = [];
        if (isset($project_designers)) {
            $_pm['project_designers'] = $project_designers;
            $designer = $this->add_edit_designer_members($_pm, $id);
        }
        $_pm = [];
        if (isset($project_technologists)) {
            $_pm['project_technologists'] = $project_technologists;
            $tech = $this->add_edit_technologist_members($_pm, $id);
        }
    ...
    public function get_project_custom_members($id, $type, $with_name = false)
    {
        ...
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'project_res_members.staff_id');
        $this->db->where('project_id', $id);
        $this->db->where('user_type', $type);
       
        return $this->db->get(db_prefix() . 'project_res_members')->result_array();
    }

    public function add_edit_mounter_members($data, $id)
    {
        $affectedRows = 0;
        if (isset($data['project_mounters'])) {
            $project_mounters = $data['project_mounters'];
        ...
        $project_members_in = $this->get_project_custom_members($id, 1);
      
        if (sizeof($project_members_in) > 0) {
            foreach ($project_members_in as $project_member) {
                if (isset($project_mounters)) {
                    if (!in_array($project_member['staff_id'], $project_mounters)) {
                        $this->db->where('project_id', $id);
                        $this->db->where('staff_id', $project_member['staff_id']);
                        $this->db->where('user_type', 1);
                        $this->db->delete(db_prefix() . 'project_res_members');
            ...
            if (isset($project_mounters)) {
                ...
                $notifiedUsers = [];
                foreach ($project_mounters as $staff_id) {
                    $this->db->where('project_id', $id);
                    $this->db->where('staff_id', $staff_id);
                    $_exists = $this->db->get(db_prefix() . 'project_res_members')->row();
                    if (!$_exists) {
                        if (empty($staff_id)) {
                            continue;
                        }
                        $this->db->insert(db_prefix() . 'project_res_members', [
                            'project_id' => $id,
                            'staff_id'   => $staff_id,
                            'user_type'  => 1,
                        ]);
    ...
    public function add_edit_designer_members($data, $id)
    {
        $affectedRows = 0;
        if (isset($data['project_designers'])) {
            $project_designers = $data['project_designers'];
        ...
        $project_name = $project->name;
        $client_id    = $project->clientid;
        $project_members_in = $this->get_project_custom_members($id, 2);
        if (sizeof($project_members_in) > 0) {
            foreach ($project_members_in as $project_member) {
                if (isset($project_designers)) {
                    if (!in_array($project_member['staff_id'], $project_designers)) {
                        $this->db->where('project_id', $id);
                        $this->db->where('staff_id', $project_member['staff_id']);
            ...
            if (isset($project_designers)) {
                $notifiedUsers = [];
                foreach ($project_designers as $staff_id) {
                    $this->db->where('project_id', $id);
                    $this->db->where('staff_id', $staff_id);
                    $this->db->where('user_type', 2);
                    $_exists = $this->db->get(db_prefix() . 'project_res_members')->row();
    ...
    public function add_edit_technologist_members($data, $id)
    {
        $affectedRows = 0;
        if (isset($data['project_technologists'])) {
            $project_technologists = $data['project_technologists'];
        ...
        $project_name = $project->name;
        $client_id    = $project->clientid;
        $project_members_in = $this->get_project_custom_members($id, 3);
        if (sizeof($project_members_in) > 0) {
            foreach ($project_members_in as $project_member) {
                if (isset($project_technologists)) {
                    if (!in_array($project_member['staff_id'], $project_technologists)) {
                        $this->db->where('project_id', $id);
                        $this->db->where('staff_id', $project_member['staff_id']);
            ...
            if (isset($project_technologists)) {
                $notifiedUsers = [];
                foreach ($project_technologists as $staff_id) {
                    $this->db->where('project_id', $id);
                    $this->db->where('staff_id', $staff_id);
                    $this->db->where('user_type', 3);
                    $_exists = $this->db->get(db_prefix() . 'project_res_members')->row();
        ...
        if ($id) {
            ...
            if (isset($data['tasks'])) {
                foreach ($tasks as $index => $task) {
                    ...
                    $statuses = json_decode($data['copy_project_task_status']);
            ...
            if (isset($data['members'])) {
                ...
                $project_mounters  = $this->get_project_custom_members($project_id,1);
                $project_designers = $this->get_project_custom_members($project_id,2);
                $project_technologists = $this->get_project_custom_members($project_id,3);
                $_project_mounters = [];
                foreach ($project_mounters as $item)
                {
                    array_push($_project_mounters, $item['staff_id']);
                }
                $_pm['project_mounters'] = $_project_mounters;
                $_project_designers = [];
                foreach ($project_designers as $item)
                {
                    array_push($_project_designers, $item['staff_id']);
                }
                $_pm['project_designers'] = $_project_designers;
                $_project_technologists = [];
                foreach ($project_technologists as $item)
                {
                    array_push($_project_technologists, $item['staff_id']);
                }
                $_pm['project_technologists'] = $_project_technologists;
                $this->add_edit_mounter_members($_pm,$id);
                $this->add_edit_designer_members($_pm,$id);
                $this->add_edit_technologist_members($_pm,$id);
            }

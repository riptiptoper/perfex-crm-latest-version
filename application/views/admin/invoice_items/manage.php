                        ...
                        <table class="table dt-table table-items-groups" data-order-col="1" data-order-type="asc">
                            ...
                            <tbody>
                                ...
                                <tr class="row-has-options" data-group-row-id="<?php echo $group['id']; ?>">
                                    ...
                                    <td data-order="<?php echo $group['offer_type']; ?>">
                                        <span class="offer_name_plain_text"><?php echo $group['offer_type']; ?></span>
                                        <div class="offer_edit hide">
                                            <div class="input-group">
                                                <select type="text" id="offer_type" class="form-control">
                                                    <?php if ($group['offer_type']) { ?>
                                                        <option value="<?php echo $group['offer_type']; ?>"><?php echo $group['offer_type']; ?></option>
                                                    <?php } ?>
                                                ...
                                                <span class="input-group-btn">
                                                    <button class="btn btn-primary update-item-offer"
                                                        type="button"><?php echo _l('submit'); ?></button>
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <td data-order="<?php echo $group['id']; ?>"><span>Mandatory </span><input <?php if ($group['is_mandatory'] == '1') { ?> checked <?php } ?> class="update-item-mandatory mandatory_edit" id="mandatory_<?php echo $group['id']; ?>" type="checkbox" ></td>
                                </tr>
                                <?php } ?>
                            </tbody>
                        </table>
...
$(function() {
    ...
    $('body').on('click', '.edit-item-offer', function(e) {
        e.preventDefault();
        var tr = $(this).parents('tr'),
            group_id = tr.attr('data-group-row-id');
        tr.find('.offer_name_plain_text').toggleClass('hide');
        tr.find('.offer_edit').toggleClass('hide');
        tr.find('.offer_edit input').val(tr.find('.group_name_plain_text').text());
    });
    ...
    $('body').on('click', '.update-item-offer', function() {
        var tr = $(this).parents('tr');
        var group_id = tr.attr('data-group-row-id');
        var offer = tr.find('.offer_edit select').val();
        if (offer != '') {
            $.post(admin_url + 'invoice_items/update_group/' + group_id, {
                offer_type: offer
            }).done(function() {
    ...
    $('body').on('change', '.update-item-mandatory', function() {
        var tr = $(this).parents('tr');
        var group_id = tr.attr('data-group-row-id');
        var id = '#mandatory_' + group_id;
        var is_mandatory = $(id).is(":checked");
        if (is_mandatory) {
            $.post(admin_url + 'invoice_items/update_group/' + group_id, {
                is_mandatory: 1
            }).done(function() {

...
<div class="modal-header task-single-header" data-task-single-id="<?php echo $task->id; ?>"
    ...
    <input hidden id="task_id" value="<?php echo $task->id; ?>">
    ...
    <div class="row">
        <div class="col-md-8 task-single-col-left">
          ...
          echo 'hide';
            ...
            <?php if (!empty($task->report_document)) { ?>
            <hr />
            <div class="clearfix"></div>
            <div>
                <p><b>Document de raportare</b></p>
                <?php
                foreach(json_decode($task->report_document) as $item)
                {
                    $this->db->where('id', $item);
                    $group = $this->db->get(db_prefix() . 'documentgroups')->row();
                    echo '<h5 id="'. $group->id .'">' . $group->name . '</h5>';
                    echo render_custom_fields_for_view_task_template('reports', true, [], [], (json_decode($group->value)), $group->id, $task->id);
                }
                ?>
            </div>
            <?php } ?>
...
function changeFieldVal(value)
{
    var params = value.split('_');
    var task_id = $('#task_id').val();
    var val = $('#'+value).val();
    $.ajax({
        url: admin_url + 'reports/save_field_data',
        method: "POST",
        data: {
            'group_id': params[0],
            'fieldid': params[1],

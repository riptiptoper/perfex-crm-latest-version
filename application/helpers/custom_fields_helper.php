 ...
 * @return mixed
 */
function render_custom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [], $include_ids = [])
{
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            if ($belongs_to == "reports" && !in_array($field['id'], $include_ids)) {
                continue;
            }
...
function render_custom_fields_for_view_task_template($belongs_to, $rel_id = false, $where = [], $items_cf_params = [], $include_ids = [], $group_id, $task_id)
{
    // Is custom fields for items and in add/edit
    $items_add_edit_preview = isset($items_cf_params['add_edit_preview']) && $items_cf_params['add_edit_preview'] ? true : false;
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            $param = $group_id . '_'. $field['id'];
            $_input_attrs['data-fieldto'] = $field['fieldto'];
            $_input_attrs['data-fieldid'] = $field['id'];
            $_input_attrs['onchange'] = "changeFieldVal('". $param ."')";

            $cf_name = $group_id . '_'. $field['id'];
            ...
            unset($_input_attrs['id']);
            $_input_attrs['id'] = $param;
            $field_name = $field['name'];
            if ($field['type'] == 'input' || $field['type'] == 'number') {
                $t = $field['type'] == 'input' ? 'text' : 'number';
                $fields_html .= render_input($cf_name, $field_name, $value, $t, $_input_attrs);
                ...
                $_select_attrs['onchange'] = "changeFieldVal('". $param ."')";
                $_select_attrs['id'] = $group_id . '_'. $field['id'];

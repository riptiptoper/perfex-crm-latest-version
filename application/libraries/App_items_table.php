...
class App_items_table extends App_items_table_template
{
    ...
    public function items()
    {
        ...
        $discount_column = false;
        foreach ($this->items as $item)
        {
            if ($item['discount'] > 0) $discount_column = true;
        }
        ...
        foreach ($this->items as $item) {
            ...
            // Table data number
            $itemHTML .= '<td' . $this->td_attributes() . ' align="center" width="10%">' . $i . '</td>';
             ...
             */
                $itemHTML .= '<span style="font-size:' . $this->get_pdf_font_size() . 'px;"><strong>'
                . $this->period_merge_field($item['manual_description'])
                . '</strong></span>';
                $itemHTML .= '</td>';
            ...
            if ($discount_column) {
                if ($item['discount'] > 0) {
                    ...
                    $itemHTML .= '<td align="right" width="' . $regularItemWidth . '%">' . $item['discount'] . ' %' .'</td>';
        
                }
                else
                    $itemHTML .= '<td align="right" width="' . $regularItemWidth . '%"></td>';
            }
                

            $item_amount_with_quantity = hooks()->apply_filters(
                'item_preview_amount_with_currency',
                app_format_money(($item['qty'] * $item['rate'] - $item['qty'] * $item['rate'] * $item['discount'] / 100), $this->transaction->currency_name, $this->exclude_currency()),
                $item,
                $this->transaction,
    ...
    public function pdf_headings()
    {
        ...
        $tblhtml .= '<th width="10%" align="left">' . $this->number_heading() . '</th>';
        ...
        $tblhtml .= '<th width="' . $descriptionItemWidth . '%" align="left">' . $this->description_heading() . '</th>';
    ...
    public function pdf_headings_coustom()
    {
        ...
        $tblhtml .= '<th width="10%;" align="center">' . $this->number_heading() . '</th>';
        $tblhtml .= '<th width="' . $descriptionItemWidth . '%" align="left">' . $this->item_heading() . '</th>';
